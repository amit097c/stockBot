...
public boolean placeLongBuyWithManualOCO(String symbol, int qty,String order_id) {
    try {
        ...
        if (responseCode != 200 && responseCode != 201) {
            ...
            System.err.println("AlpacaApiClient::placeLongSellWithManualOCO:1164 Failed to place long buy order. Response code: " + responseCode);
            return false;
        }
...
public String placeLimitBuy(String symbol, int qty, double limitPrice) {
    ...
    try {
        ...
        order.put("client_order_id", order_id); // Add client order ID
          ...
          int responseCode = conn.getResponseCode();
if (responseCode == 200 || responseCode == 201) {
    System.out.println("AlpacaApiClient:placeStopBuy:1288 limitPrice Long order placed: " + limitPrice);
    return order_id;
} else {
    InputStream errorStream = conn.getErrorStream();
    if (errorStream != null) {
