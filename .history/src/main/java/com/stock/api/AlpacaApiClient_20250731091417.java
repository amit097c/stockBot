...
public boolean placeShortSellWithManualOCO(String symbol, int qty, double stopLossPrice, double takeProfitPrice,String order_id) {
    try {
        ...
        System.out.println("AlpacaApiClient::placeShortSellWithManualOCO:1112 Short SELL executed.");
...
public boolean placeLongBuyWithManualOCO(String symbol, int qty,String order_id) {
    try {
        ...
        System.out.println("AlpacaApiClient::placeLongSellWithManualOCO:1112 Long buy executed.");
...
/*public boolean placeLongBuyOCO(String symbol, int qty, double stopLossPrice, double takeProfitPrice, String clientOrderId) {
    try {
        System.out.println("Placing Long Buy Bracket Order...");
        System.out.println("Symbol: " + symbol + ", Qty: " + qty + ", Client Order ID: " + clientOrderId);

        // Simulate or fetch current market price
        double currentPrice = getMarketPrice(symbol);  // Replace with actual price-fetch logic
        System.out.println("Current Market Price: " + currentPrice);

         double stopLossLimitPrice = Math.round((stopLossPrice - 0.20) * 100.0) / 100.0;

        System.out.println("Take Profit Price: " + takeProfitPrice);
        System.out.println("Stop Loss Trigger Price: " + stopLossPrice);
        System.out.println("Stop Loss Limit Price: " + stopLossLimitPrice);
        ...
        JSONObject stopLoss = new JSONObject();
        stopLoss.put("stop_price", stopLossPrice);
        stopLoss.put("limit_price", stopLossLimitPrice);
        ...
        System.out.println("Constructed Order JSON:\n" + order.toString(2));
        ...
        try (OutputStream os = conn.getOutputStream()) {
            os.write(order.toString().getBytes(StandardCharsets.UTF_8));
            System.out.println("Order JSON sent to Alpaca.");
        }

        int responseCode = conn.getResponseCode();
        System.out.println("Alpaca API Response Code: " + responseCode);
        ...
        if (responseCode == 200 || responseCode == 201) {
            ...
            System.err.println("Order Placement Failed:\n" + errorResponse.toString());
        }

    } catch (Exception e) {
        System.err.println("Exception while placing long buy bracket order:");
        e.printStackTrace();
    }
...
public  String placeLimitSellForLong(String symbol, int qty, double limitPrice) {
    ...
    try {
        ...
        if (responseCode == 200 && responseCode == 201) {
            ...
            System.out.println("AlpacaApiClient:placeStopBuy:1356 Stop Sell Long order placed: " + limitPrice);
 
            return order_id; // Indicate failure
        }
        else
         {
            // Log error response body
                BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getErrorStream()));
                String line;
                System.err.println("Attempt " + attempt + " failed to place limit sell order (HTTP " + responseCode + "):");
                while ((line = reader.readLine()) != null) {
                    System.err.println(line);
                }
         }
...
public String placeLimitBuy(String symbol, int qty, double limitPrice) {
    ...
    try {
        ...
        if (responseCode == 200 && responseCode == 201) {
         ...
         System.out.println("AlpacaApiClient:placeStopBuy:1584 Limit BUY (TP) order placed: " + limitPrice);
        ...
        else
         {
            // Log error response body
                BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getErrorStream()));
                String line;
                System.err.println("Attempt " + attempt + " failed to place limit buy order (HTTP " + responseCode + "):");
                while ((line = reader.readLine()) != null) {
                    System.err.println(line);
                }
         }
