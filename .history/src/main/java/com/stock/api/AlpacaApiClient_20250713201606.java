...
public void placeBracketOrder(String symbol, int qty, double stopLossPrice, double takeProfitPrice) {
    try {
        ...
        stopLoss.put("stop_price", String.valueOf(stopLossPrice));
        stopLoss.put("limit_price", String.valueOf(stopLossPrice - 0.20)); // buffer below stop to ensure exit

        JSONObject takeProfit = new JSONObject();
        takeProfit.put("limit_price", String.valueOf(takeProfitPrice));
        ...
        System.out.println("AlpacaApiClient::placeMarketOrder:335 Bracket order placed: " + response.toString());
    } catch (Exception e) {
        System.err.println("AlpacaApiClient::placeMarketOrder:337 Error placing bracket order for " + symbol + ": " + e.getMessage());
        e.printStackTrace();
    }
}
