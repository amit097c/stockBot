...
public class AlpacaApiClient 
{
    ...
    public LocalDate getPreviousWeekDay() {
        ...
        return today.minusWeeks(0).with(DayOfWeek.WEDNESDAY); // Adjust to previous DAY
    }
public StockBar[] fetch1MinBars(String symbol) 
     {
        
          System.out.println("AlpacaApiClient::fetchLatest1MinBar:82 Fetching data for date: " + symbol);
          StockBar latestBar = null;
          StockBar previousBar = null;
          StockBar[] all_bars=new StockBar[2];
          try {
               // String queryParams = String.format("symbols=%s&feed=iex", URLEncoder.encode(symbol, "UTF-8"));
                String queryParams = String.format("timeframe=1Min&limit=2&sort=desc&feed=iex",URLEncoder.encode(symbol, StandardCharsets.UTF_8.name())); 
               URL url = new URL("https://data.alpaca.markets/v2/stocks/{{SYMBOL}}/bars?" + queryParams);
                ...
                //JSONObject bar = bars.getJSONObject(symbol);
                JSONArray bars_arr = json.getJSONArray("bars");
                if (bars_arr.length() < 2) {
                    System.out.println("AlpacaApiClient:fetchLatest1MinBar:130: Not enough bars returned for " + symbol + " to get previous.");
                    return null;
                }

                JSONObject prev = bars_arr.getJSONObject(1);
                JSONObject bar=bars_arr.getJSONObject(0);
                latestBar = new StockBar(
                    symbol,
                    bar.getDouble("o"),
                    bar.getDouble("h"),
                ...
                previousBar = new StockBar(
                    ...
                    prev.getDouble("c"),
                    prev.getLong("v"),
                    prev.getString("t"),         // timestamp (ISO format)
                    prev.optInt("n", 0),         // number of trades (optional)
                    prev.optDouble("vw", 0.0),   // volume-weighted price (optional)
                    TimeUtils.getCurrentDateTime()
                );
                all_bars[0]=latestBar;
                all_bars[1]=previousBar;
...
public OcoFill waitForOcoFillByClientId(String ocoClientOrderId, long timeoutMs, long pollMs) throws InterruptedException {
    ...
    if (slId == null || tpId == null) {
        System.out.println("AlapacaApiClient:waitForOcoFillByClientId:591: OCO legs not found for client_order_id=" + ocoClientOrderId);
        return null;
    }
...
//to do: ensure sell order is there before placing the opposite call-introduce guards

private int getOpenShortQty(String symbol) throws Exception {
    JSONObject pos = httpGetJson("https://paper-api.alpaca.markets/v2/positions/" + symbol);
    if (pos == null) return 0; // 404 -> no position
    String side = pos.optString("side", "");
    // Alpaca returns qty as string
    int qty = Integer.parseInt(pos.getString("qty"));
    return "short".equalsIgnoreCase(side) ? qty : 0;
}
private int getOpenLongQty(String symbol) throws Exception {
    JSONObject pos = httpGetJson("https://paper-api.alpaca.markets/v2/positions/" + symbol);
    if (pos == null) return 0; // 404 -> no position
    String side = pos.optString("side", "");
    // Alpaca returns qty as string
    int qty = Integer.parseInt(pos.getString("qty"));
    return "long".equalsIgnoreCase(side) ? qty : 0;
}
...
public String placeOcoBuyToClose(String symbol, int qty, double takeProfitLimitBuy, double stopLossBuy,double shortSellEntryPrice)
 {
    try{
        ...
        if(stopLossBuy>shortSellEntryPrice && takeProfitLimitBuy<shortSellEntryPrice)
        {
            ...
            JSONObject stopLoss = new JSONObject();
            stopLoss.put("stop_price", round(stopLossBuy,2));
            stopLoss.put("limit_price", round(((stopLossBuy + 0.20)),2));  // buffer above stop price
            JSONObject takeProfit = new JSONObject();
            takeProfit.put("limit_price", takeProfitLimitBuy);
            int open_qty=0;
            do{
                 open_qty=getOpenShortQty(symbol);
                 System.out.println("AlpacaApiClient::placeOcoBuyToClose:1018 Checking open short qty for "+symbol+" open_qty: "+qty+" Thread: "+Thread.currentThread().getName()+" going to sleep for 1 second");
                 Thread.sleep(1000); // wait for 1 second before checking again
            }while(open_qty==0); 
            
            System.out.println("AlpacaApiClient::placeOcoBuyToClose:1022 Open short qty for "+symbol+" is: "+open_qty+" Thread: "+Thread.currentThread().getName());
            if(Math.abs(open_qty)<qty)
            {
                System.err.println("AlpacaApiClient::placeOcoBuyToClose:1025 Open short qty for "+symbol+" is less than requested qty: "+open_qty+" < "+qty+" Thread: "+Thread.currentThread().getName());
                qty=Math.abs(open_qty); // use the open short qty
            }  
            ...
            int responseCode = conn.getResponseCode();
            if (responseCode == 200 || responseCode == 201) {
                System.out.println("AlpacaApiClient::placeOcoBuyToClose:1048 Order placed successfully order_id: "+order_id);
                return order_id;
            }
            else {       
                BufferedReader in = new BufferedReader(new InputStreamReader(conn.getErrorStream()));
                ...
                in.close();
                System.err.println("AlpacaApiClient::placeOcoBuyToClose:1059: Alpaca API error (" + responseCode + "): " + error);
                return "";
               }       
        }   
        else
        {
            System.err.println("AlpacaApiClient::placeOcoBuyToClose:1065 Stop loss buy or take profit limit buy is not in range of short sell entry price. stopLossBuy: "+stopLossBuy+" takeProfitLimitBuy: "+takeProfitLimitBuy+" shortSellEntryPrice: "+shortSellEntryPrice);
            return "";
        }
    ...
    {
        System.err.println("AlpacaApiClient::placeOcoBuyToClose:1077 Exception in placeOcoBuyToClose: "+e.getMessage());
    }
    return "";
 }
public String placeOcoSellToClose(String symbol, int qty, double takeProfitLimitSell, double stopLossSell,double longBuyEntryPrice)
 {
   ...
   /*  try{
        ...
        if(stopLossSell<longBuyEntryPrice && takeProfitLimitSell>longBuyEntryPrice)
        {
            System.out.println("AlpacaApiClient::placeOcoSellToClose:1090 Stop loss buy and takeProfit limit is in range of long buy entry price. stopLossBuy: "+stopLossSell+" takeProfitLimitBuy: "+takeProfitLimitSell+" shortSellEntryPrice: "+longBuyEntryPrice);
            String order_id = generateClientOrderId(symbol, "sell_to_close");
            int open_qty=0;
            do
             {
                open_qty=getOpenLongQty(symbol);
                System.out.println("AlpacaApiClient::placeOcoSellToClose:1100 Checking open long qty for "+symbol+" open_qty: "+open_qty+" Thread: "+Thread.currentThread().getName()+" going to sleep for 1 second");
                Thread.sleep(1000); // wait for 1 second before checking again
             }while(open_qty==0);
             qty=Math.min(qty, open_qty);

            JSONObject stopLoss = new JSONObject()
            .put("stop_price", stopLossSell)
            // stop-limit optional; for SELL, make limit <= stop to improve fill chance
            .put("limit_price", round(Math.max(0.01, stopLossSell - 0.20),2));

            JSONObject takeProfit = new JSONObject()
            .put("limit_price", round(takeProfitLimitSell,2));
            ...
            int responseCode = conn.getResponseCode();
            if (responseCode == 200 || responseCode == 201) {
                System.out.println("AlpacaApiClient::placeOcoSellToClose:1116 Order placed successfully order_id: "+order_id);
                return order_id;
            }
            else {       
                BufferedReader in = new BufferedReader(new InputStreamReader(conn.getErrorStream()));
                ...
                in.close();
                System.err.println("AlpacaApiClient::placeOcoSellToClose:1127: Alpaca API error (" + responseCode + "): " + error);
                return "";
               }       
        }   
        else
        {
            System.err.println("AlpacaApiClient::placeOcoSellToClose:1142: Stop loss buy or take profit limit buy is not in range of short sell entry price. stopLossBuy: "+stopLossSell+" takeProfitLimitBuy: "+takeProfitLimitSell+" shortSellEntryPrice: "+longBuyEntryPrice);
            return "";
        }
...
public String  placeLongEntry(String symbol, int qty,String type,double limitPrice)
 {
    ...
    /*try{
        ...
        int responseCode = conn.getResponseCode();
         if (responseCode == 200 || responseCode == 201) {
            System.out.println("AlpacaApiClient::placeLongEntry:1310 long entry order place for : " + symbol+ " order_id: " + order_id+" "+Thread.currentThread().getId());
        ...
        else 
         {
            ...
            System.out.println("AlpacaApiClient::placeLongEntry:1320 Failed to place long buy order. Response code: " + responseCode+ " for symbol: " + symbol+ "order_id: " + order_id);
            return ""; 
         } 
    }
   catch(Exception e) {
...
public String generateClientOrderId(String symbol, String side) {
    ...
    return String.format("%s-OCO-%s-%s-%s", symbol, side.toUpperCase(), timestamp,rand);
}
