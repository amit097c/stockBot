...
public int placeSellBracketOrder(String symbol, int qty, double targetEntry, double stopLossHigh, double takeProfitHigh) {
    try {
        double entryPrice = getLastTradePrice(symbol); // Fetch current market price
           
        System.out.printf("ApiClient:708 Entry price for %s: %.4f%n", symbol, entryPrice);

        double stopPrice = Math.max(stopLossHigh, entryPrice + 0.01); // Ensure stop > entry
        double takePrice = Math.min(takeProfitHigh, entryPrice - 0.01); // Ensure take < entry

        JSONObject stop = new JSONObject();
        stop.put("stop_price", stopPrice);
        stop.put("limit_price", stopPrice + 0.25); // Conservative slippage

        JSONObject take = new JSONObject();
        take.put("limit_price", takePrice);
        ...
        int responseCode = conn.getResponseCode();
        System.out.println("ApiClient:744 Response Code: " + responseCode);
        if (responseCode != 200 && responseCode != 201) {
            throw new IOException("Order rejected. Code: " + responseCode);
        }

        System.out.println("Sell bracket order placed for: " + symbol);
        return 1;
    } catch (Exception e) {
        System.err.println("ApiClient:686: Error placing sell bracket: " + e.getMessage());
        e.printStackTrace();
        return -1;
    }
}
public int placeSellBracketOrder(String symbol, int qty, double stopLoss, double takeProfit,String order_id) {
    try {
        URL url = new URL("https://paper-api.alpaca.markets/v2/orders");
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("POST");
        ...
        legs.put("take_profit", take);
        legs.put("stop_loss", stop);
