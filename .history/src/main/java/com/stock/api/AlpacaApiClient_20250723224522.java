...
public int placeShortSellOrder(String symbol, int qty, double stopLoss, double takeProfit) {
    try {
        URL url = new URL("https://paper-api.alpaca.markets/v2/orders");
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("POST");
        ...
        JSONObject take = new JSONObject();
        take.put("limit_price", round(takeProfit, 2));

        JSONObject stop = new JSONObject();
        stop.put("stop_price", round(stopLoss, 2));
        stop.put("limit_price", round(stopLoss + 0.25, 2));  // buffer above
        ...
        int responseCode = conn.getResponseCode();
        if (responseCode == 200 || responseCode == 201) {
            System.out.println("Short sell order placed: " + symbol);
        } else {
            BufferedReader in = new BufferedReader(new InputStreamReader(conn.getErrorStream()));
            String line;
            while ((line = in.readLine()) != null) {
...
public void placeShortSellOrder(String symbol, int qty,double stopLoss, double takeProfit) {
    try {  
