...
public String placeOcoBuyToClose(String symbol, int qty, double takeProfitLimitBuy, double stopLossBuy,double shortSellEntryPrice)
 {
    try{
        ...
        if(stopLossBuy<=shortSellEntryPrice || takeProfitLimitBuy>=shortSellEntryPrice)
         {
            if(stopLossBuy<=shortSellEntryPrice )
             {
                stopLossBuy+= 0.20; // buffer above stop price
                System.out.println("AlpacaApiClient::placeOcoBuyToClose:907 Stop loss buy is below short sell entry price. Adjusting stopLossBuy to: "+stopLossBuy);
             }
            if(takeProfitLimitBuy>=shortSellEntryPrice)
             {
                takeProfitLimitBuy-= 0.20; // buffer below take profit price
                System.out.println("AlpacaApiClient::placeOcoBuyToClose:907 Take profit limit buy is above short sell entry price. Adjusting takeProfitLimitBuy to: "+takeProfitLimitBuy);
             }
         }
        if(stopLossBuy>shortSellEntryPrice && takeProfitLimitBuy<shortSellEntryPrice)
        {
            ...
            order.put("take_profit", new JSONObject().put("limit_price", takeProfitLimitBuy));
            order.put("stop_loss", new JSONObject().put("stop_price", stopLossBuy)
                .put("limit_price", stopLossBuy + 0.20)); // buffer above stop price    
            order.put("time_in_force", "day");
            order.put("client_order_id",order_id);
            try (OutputStream os = conn.getOutputStream()) {
                os.write(order.toString().getBytes(StandardCharsets.UTF_8));
    ...
    {
        System.err.println("Exception in placeOcoBuyToClose: "+e.getMessage());
    }
 }
